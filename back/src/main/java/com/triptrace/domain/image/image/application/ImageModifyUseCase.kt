...
@Service
class ImageModifyUseCase(private val imageService: ImageService, private val tripService: TripService, private val postService: PostService, private val memberService: MemberService) {
    @Transactional
    fun modifyById(ownerId: Long, tripId: Long, postId: Long, imageId: Long): ImageServiceResponse {
        val owner = memberService.findById(ownerId); val trip = tripService.findOwnedTrip(tripId, owner.id); val post = postService.getPost(trip, postId)
    ...
    fun unassign(ownerId: Long, tripId: Long, imageId: Long) = imageService.unassign(ownerId, tripId, imageId)
}
