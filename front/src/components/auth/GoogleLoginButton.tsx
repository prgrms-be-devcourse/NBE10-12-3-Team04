...
import { buildGoogleAuthUrl, saveOAuthState } from '@/lib/oauth';
...
export default function GoogleLoginButton({ disabled }: { disabled?: boolean }) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={disabled}
      className="flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white py-2.5 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-60"
    >
      <GoogleMark />
      구글로 로그인
    </button>
